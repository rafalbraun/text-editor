...
int search_process(gchar* needle, gchar* path) {
  char *argv [15];
  GPid child_pid;
  int status;
  ...
  if (stdout_fd == -1) {
    g_print("[FAILED][%s] 3 out fd is -1 \n", g_get_application_name ());
    return 1;
  }
...
key_pressed(GtkWidget *entry, GtkWidget* listview) {
    ...
    if (strlen(needle) > gLimit) {
        search_process(needle, "/usr/include");
        search_process(needle, "/usr/local/share");
        search_process(needle, "/usr/share");
    }
...
int main(int argc, char *argv[]) {
  ...
  gtk_window_set_title(GTK_WINDOW(window), "Find Files");
