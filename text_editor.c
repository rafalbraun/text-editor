...
*/
  ...
  g_file_set_contents (((UserData *) userdata)->session_info, contents,
		       strlen (contents), &err);
  g_free (contents);
...
static void
show_subwindow (GtkButton * widget, gpointer user_data)
{
  ...
  g_signal_connect (G_OBJECT (subwindow), "destroy", G_CALLBACK (gtk_widget_destroy),
		    NULL);
...
void set_language(GObject* buffer) {
  ...
  lang_dirs[1] =
    "/home/rafal/.local/share/flatpak/exports/share/gtksourceview-4/language-specs ";
...
main (int argc, char *argv[])
{
  ...
  gtk_text_buffer_create_tag (GTK_TEXT_BUFFER (buffer), "blue", "background",
			      "white", NULL);
  gtk_text_buffer_create_tag (GTK_TEXT_BUFFER (buffer), "black", "foreground",
			      "black", NULL);
  gtk_text_buffer_create_tag (GTK_TEXT_BUFFER (buffer), "italic", "style",
  ...
  gtk_notebook_set_action_widget (GTK_NOTEBOOK (notebook), action_widget,
				  GTK_PACK_END);
  ...
  g_signal_connect (G_OBJECT (window), "destroy", G_CALLBACK (on_main_quit),
		    (gpointer) userdata);
  g_signal_connect (G_OBJECT (window), "key-press-event",
		    G_CALLBACK (key_pressed_window), NULL);
  g_signal_connect (G_OBJECT (treeview), "key-press-event",
		    G_CALLBACK (key_pressed_treeview), NULL);
  g_signal_connect (G_OBJECT (treeview), "button-press-event",
		    G_CALLBACK (on_button_pressed), (gpointer) userdata);
  g_signal_connect (G_OBJECT (notebook), "switch-page", G_CALLBACK (switch_page),
		    (gpointer) userdata);

  g_signal_connect (G_OBJECT (window), "key-press-event",
		    G_CALLBACK (key_pressed_notebook), NULL);
